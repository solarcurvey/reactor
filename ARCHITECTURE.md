...
- **ValuationService:** one recursive CAT→ZCAT→ZEC→USD path with ancestry. Parent-only USD is rejected. Official Top-10 ranks are the persisted `GET /top10` snapshot (schema v10) from `tokens.current_supply`. Official Top-10 ranks are the persisted `GET /top10` snapshot (schema v10) from `tokens.current_supply`.
- **Web:** homepage is `GET /markets` (zero per-token RPC). `/api/reactor/top10` proxies indexer `GET /top10`. Trades use the quote API — no wallet hop sim for missing intermediate assets.
