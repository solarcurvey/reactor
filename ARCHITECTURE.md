...
- **Web:** homepage is `GET /markets` (zero per-token RPC). Official Top-10 is indexer `GET /top10` (schema v11 snapshot, 15m TTL shared with Keeper) — no per-request Factory walk and no invented 0.30% hookless pool. Trades use the quote API — no wallet hop sim for missing intermediate assets.
