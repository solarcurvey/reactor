...
export function Nav() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-rx-steel bg-rx-slag/90 backdrop-blur-xl">
          ...
          {links.map((l) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "shrink-0 rounded-[var(--rx-radius-chip)] px-2 py-1.5 text-rx-cool hover:text-rx-paper sm:px-3",
                  l.wide && "hidden sm:inline-flex",
                  active && "bg-rx-steel text-rx-paper",
                )}
              >
                {l.label}
              </Link>
