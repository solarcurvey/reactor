  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <input
        ...
        className="mt-4 h-11 w-full rounded-[4px] border border-white/10 bg-black/30 px-4 text-sm outline-none"
        autoFocus
      />
      <div className="mt-3 flex gap-2">
        {(["all", "bonding", "v4"] as const).map((s) => (
          <button
            ...
            onClick={() => setStage(s)}
            className={`rounded-[2px] px-3 py-1 text-[12px] font-semibold ${stage === s ? "bg-rx-paper text-rx-slag" : "bg-white/5 text-zinc-400"}`}
          >
            {s}
          </button>
        ))}
      ...
      <ul className="mt-4 divide-y divide-white/8 rounded-[4px] border border-white/8">
        {items.map((t) => (
          <li key={t.token}>
            <Link href={tokenPath(t.token)} className="flex items-center justify-between px-4 py-3 hover:bg-white/[0.03]">
              ...
              <span className="text-[11px] font-semibold uppercase tracking-wider text-rx-ember">
