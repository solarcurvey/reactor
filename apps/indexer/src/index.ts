...
const events = [
  ...
  parseAbiItem("event FairLaunchCreated(uint256 indexed fairId, address indexed token, uint64 startTime, uint64 endTime)"),
  parseAbiItem("event FairLaunchFinalized(uint256 indexed fairId, uint256 totalBids, uint256 auctionTokens)"),
...
async function tick() {
  ...
  const insertSw = db.prepare("INSERT INTO swaps(block,tx,token,quote,holders,buyback,notional) VALUES(?,?,?,?,?,?,?)");
...
const server = createServer((req, res) => {
  ...
  if (swapMatch) {
    const rows = db
      .prepare("SELECT block as t, notional, holders, buyback, tx FROM swaps WHERE lower(token)=lower(?) ORDER BY id ASC")
      .all(swapMatch[1]);
    res.end(JSON.stringify(rows));
    return;
  }
