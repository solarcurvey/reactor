...
# This helper deliberately changes Lambda code only. The OIDC role in
# infra/aws-relay has no IAM/KMS/config/pass-role authority.
for suffix in maintenance-authorizer relay-a relay-b; do
  fn="${PREFIX}-${suffix}"
  echo "deploying reviewed code to $fn"
...
echo "AWS relay code deployed to all three functions; IAM/KMS/runtime configuration unchanged."
