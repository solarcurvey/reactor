...
- Exact official-list digital-currency address screening (`@reactor/sanctions`, `GET /sanctions/screen`). Official Treasury/OFAC HTTPS XML only (default refresh: SDN + Consolidated, classic and advanced). Decision is `blocked` / `clear` / `unavailable` plus dataset version — not a boolean, not legal/OFAC compliance, not hop attribution, not a #60 policy gate. Completeness floor 85% (explicit shrink override only). Refs #61 (parent #60). Economics unchanged.
