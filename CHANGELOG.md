...
Production browser + wallet E2E (#35) rebase onto `main` `35552f6` (#79 Replay #17 CI evidence after #68). UI QA gate (issue #36 **closed** after #49) plus indexed read-path / page-budget (issue #37 **closed** after #50) and full GitHub CI extras (#17 product-merged via #42; accepted `11fdadb` / `34727535121` then `80d3cac` / `34727638255`; **#17 stays open** until post-merge docs/CI after #79). Exact official-list OFAC screening (issue #61 **closed** after #66 `d08aa1c` / `34731099571`). Trusted geo (issue #63 **closed** after #67 `e712617` / `34731788819`, founder closed after post-merge verify). Operator policy gate (issue #62 **closed** after #68 `2002aed` / `34733128955`). Parent **#60** and **#64 / #65 / #69** stay open. **#35 stays open** until merge + post-merge verify. Public launchpad treats token metadata as untrusted (#41 / #47). Top-10 ranks move to the canonical indexer ValuationService (issue #10 / PR #33). Tokenomics **unchanged**. Factory **V1**. No mainnet. Production browser + wallet E2E (#35) rebase onto `main` `d08aa1c` (#66 OFAC screening after #49 / #42 / #50 / #58). UI QA gate (#36) and indexed read-path / page-budget (#37) stay on main. Full GitHub CI extras (#17) landed on `80d3cac`. Production browser + wallet E2E (#35) rebase onto `main` `e712617` (#67 trusted geo after #66 / #49 / #42 / #50 / #58). UI QA gate (#36), exact official-list screening (#61 / #66), and indexed read-path / page-budget (#37) stay on main. Production browser + wallet E2E (#35) rebase onto `main` `2002aed` (#68 operator-policy after #67 / #66 / #49 / #42 / #50 / #58). UI QA gate (#36), exact official-list screening (#61 / #66), operator-policy (#62 / #68), and indexed read-path / page-budget (#37) stay on main.
...
- Full GitHub CI extras (Refs #17) land on the #69 three-tier `ci.yml` after #73: `pnpm docs:links` (fast via `test:lib`; full-only job `docs-links`), Playwright smoke + interactive (full-only job `web`), and `scripts/safe-genesis-builder.test.ts` on the cheap `test:lib` path. Foundry Attack / CREATE2 / size-guard, backend unit, Postgres, production Next security, and live-toasts stay on the folded #73 jobs. PR #49 folds `web-qa.yml` into `ci.yml` job `web-qa` (full/main). Accepted #17 / #42 evidence: merge-candidate `11fdadb` / [`34727535121`](https://github.com/solarcurvey/reactor/actions/runs/34727535121), post-merge `80d3cac` / [`34727638255`](https://github.com/solarcurvey/reactor/actions/runs/34727638255). **#17 stays open** until the docs evidence PR merges + post-merge docs/CI. Not a second `push` + `pull_request` workflow. Tokenomics unchanged. - #35 E2E mock indexer (`e2e/harness/mock-backend.mjs`) serves public `#68` `GET /operator-policy/challenge` and `GET /operator-policy/status` so production-build Quote / Launch Instant / upload can attach `x-reactor-wallet-proof` after the #68 rebase. The EIP-1193 fixture auto-signs `personal_sign`; the mock does not verify the signature. The EIP-1193 fixture auto-signs `personal_sign`; `rejectTx` rejects Confirm-buy / `eth_sendTransaction` only (not the challenge). The MV3 extension Confirm after Quote covers the proof, then Confirm covers the trade. The mock does not verify the signature. iPhone WebKit reports the same access-control `pageerror` on mock `:18448` `/markets?limit=80` as `/stream`; the console-gate allowlist is those named public GET/SSE paths plus `/quote-assets` and `/operator-policy/{challenge|status}` — not a generic `:18448` or 404 allowlist. WebKit console-gate stays pinned to exact `/127.0.0.1:18448/stream` pageerror only. The home+launch journey waits for mock `GET /markets` before leaving `/` so iPhone WebKit does not abort that fetch and mis-report it as CORS (run 34734308703). Mock JSON responses keep the echoed request `Origin` (`writeHead(status)` after `setHeader`, so Node does not wipe CORS on `/markets` and other indexed GETs). Console-gate stays stream-only for WebKit EventSource. The home+launch journey waits for mock `GET /markets` before leaving `/` so iPhone WebKit does not abort that fetch (run 34734308703).
