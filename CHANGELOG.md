...
- Replayed onto main `922f909` (squash-merged **#48** handbook after **#80** Industrial Forge / Refs #55 after **#81** `ff444cb` / **#46** `789eb5c` observability / **#75** / **#44**). Regenerated `docs/llms.txt` + `apps/web/public/llms.txt` so they list protocol **0.4.0** (the #48 rebase had left handbook `0.3.4`). Playwright `e2e/protocol-version.ts` reads `docs/version.json` via cwd only (CJS-safe). Frozen fail-close ranking is preserved. Privileged Keeper EOA is **not** restored. Factory **V1** / economics unchanged. Merged #48 handbook, #80 Direction C chrome/assets, #46 `obs-ui` / redacted telemetry / error boundaries, #75 `/restricted` + official `GET /operator-policy/status`, #44 `e2e-release-gate`, #70 freshness SLA, #68 operator-policy, #67 geo, and #66 `@reactor/sanctions` stay on main. **#17 / #39 / #51 / #60 / #63 / #64 / #65 / #69 / #83 stay open.** **#39 is open/reopened** — merged #46 landed repo CI; close still requires the real configured telemetry-provider symbolication proof. **#35** closed after merged #44. **#55** closed/completed on `main` after the #48 post-merge handbook/visual proof. Refs #51 only — do not close. Issue **#51 stays open** through the real **#83** autonomous testnet/failover proof **and** post-deploy `Guardian.keeper == AutomationGateway`. Authenticated CRE simulate is **optional interoperability**, not a #54 merge blocker.
- CRE courier uses the same signed `MaintenanceJob` interface (`ops/cre/maintenance-courier`). Official `cre workflow build` (CLI v1.33.0, no tenant) compiled that courier to WASM for catalog `arc-testnet` / EIP-155 1883 (`ops/cre/simulation/cre-workflow-build.json`). Authenticated `cre workflow simulate` still needs a CRE tenant (`CRE_API_KEY` / `cre login`) and remains unrun (`ops/cre/simulation/cre-tenant-blocker.json` `closed:false`). That is **optional interoperability evidence**, not the production execution path. Production autonomous path is **#83** (AWS KMS authorizer + dual managed relays). Live DON on EIP-155 1883 is not claimed. Arc Mainnet (5042) CRE production writes are **not listed, not configured, and not claimed**.
...
- Top-10 ranks remain an offchain API. CRE is an optional courier. Production autonomous path is #83 (KMS + dual managed relays).
...
Founder-locked **Industrial Forge** brand (Direction C, issue #55 — **stays open** for independent audit) applied to production UI and launch assets. Rebased onto main **`ff444cb`** after squash-merged **#81** / #69 (docs-only path filter) and **#46** / #39 (observability). Restricted-access UX squash-merged as **#75** (`4207356` / #65). Production browser + wallet E2E (#35) squash-merged as **#44** (`b190e86`) after #70 sanctions ops (`cc82cd4` / #79 / `35552f6` / #68). UI QA gate (issue #36 **closed** after #49) plus indexed read-path / page-budget (issue #37 **closed** after #50) and full GitHub CI extras (#17 product-merged via #42; accepted `11fdadb` / `34727535121` then `80d3cac` / `34727638255`; **#17 stays open** until post-merge docs/CI after #79). Exact official-list OFAC screening (issue #61 **closed** after #66 `d08aa1c` / `34731099571`). Trusted geo core accepted on #67 `e712617` / `34731788819`; issue **#63 reopened** until #65 user-visible restricted state via PR #75. Operator policy gate (issue #62 **closed** after #68 `2002aed` / `34733128955`). Sanctions freshness/audit/alerts/runbook (issue **#64 stays open** after #70 `cc82cd4`). Parent **#60** and **#64 / #65 / #69** stay open. **#35 stays open** until post-merge verify. Public launchpad treats token metadata as untrusted (#41 / #47). Top-10 ranks move to the canonical indexer ValuationService (issue #10 / PR #33). Tokenomics **unchanged**. Factory **V1**. No mainnet.

### Added / Changed

- Canonical REACTOR identity is Industrial Forge: slag / steel / heat tokens, vessel + hexagonal ember mark, production favicon / app icon / OG / wordmark assets. THE REACTOR and CORE stay the same family with distinct chrome. After #75, `/restricted` uses the same system (heat `rx-kicker`, 0–4px radius, no cyan); amber deny banners stay semantic warnings. After #46, route/app/global error boundaries keep release SHA + `traceId` and use heat kickers (no leftover cyan/pills). Product title/description say **token launch on Arc**. Metadata distinguishes **Rewards pay holders; Standard burns** — it does not say every launch pays holders. The only allowed “AI” string is first-use metadata `Not an AI trading terminal.`; chrome and OG stay AI-free. Handbook: [Brand](docs/brand.md). **Refs #55 #36 #15 #18.** #55 stays open. #36 is closed.
...
- Page-level request/RPC budget CI (`page-budget.test.ts`) seeds 4,000 markets and asserts O(1)/O(page) HTTP + RPC waves across home, search, token terminal, Launch, Rewards, THE REACTOR, CORE, quote ecosystem, and wallet. Rapid search/filter/token/account/route changes abort obsolete fetches. SSE live prints patch TanStack cache and must not `invalidateQueries` the board. Expensive reads set `refetchOnWindowFocus: false`. Required GitHub job `page-budget` is always-on in `.github/workflows/ci.yml` (every PR, including drafts) and is required by `ci-ok`. **Refs #37** — closed after #50 post-merge `e5fd745` / [`34727279555`](https://github.com/solarcurvey/reactor/actions/runs/34727279555).

### CI / ops

- Docs-only / trivial ready-for-review PRs no longer set `FULL=true` (Refs #69 residual after #73). `scripts/ci-decide.sh` keeps those SHAs on the cheap gate; label `ci-full`, `workflow_dispatch` tier **full**, and `push` to `main` still force every required job. `scripts/ci-ok.sh` rejects skipped Solidity / Postgres / browser / `obs-ui` jobs on that full path and accepts those skips on the cheap path. Deterministic proof is `pnpm test:ci-cost`. #39 `obs-ui` is on `main` via merged #46; **#69 stays open** until one green protected-main full run. Tokenomics unchanged.
- Full GitHub CI extras (Refs #17) land on the #69 three-tier `ci.yml` after #73: `pnpm docs:links` (fast via `test:lib`; full-only job `docs-links`), Playwright smoke + interactive (full-only job `web`), and `scripts/safe-genesis-builder.test.ts` on the cheap `test:lib` path. Foundry Attack / CREATE2 / size-guard, backend unit, Postgres, production Next security, and live-toasts stay on the folded #73 jobs. PR #49 folds `web-qa.yml` into `ci.yml` job `web-qa` (full/main). Accepted #17 / #42 evidence: merge-candidate `11fdadb` / [`34727535121`](https://github.com/solarcurvey/reactor/actions/runs/34727535121), post-merge `80d3cac` / [`34727638255`](https://github.com/solarcurvey/reactor/actions/runs/34727638255). **#17 stays open** until post-merge docs/CI after #79. Not a second `push` + `pull_request` workflow. Tokenomics unchanged.
- #35 E2E mock indexer (`e2e/harness/mock-backend.mjs`) serves public `#68` `GET /operator-policy/challenge` and `GET /operator-policy/status` so production-build Quote / Launch Instant / upload can attach `x-reactor-wallet-proof` after the #68 rebase. The EIP-1193 fixture auto-signs `personal_sign`; `rejectTx` rejects Confirm-buy / `eth_sendTransaction` only (not the challenge). The MV3 extension Confirm after Quote covers the proof, then Confirm covers the trade. The mock does not verify the signature. Mock JSON responses keep the echoed request `Origin` (`writeHead(status)` after `setHeader`, so Node does not wipe CORS on `/markets` and other indexed GETs). Console-gate stays stream-only for WebKit EventSource. The home+launch journey waits for mock `GET /markets` before leaving `/` so iPhone WebKit does not abort that fetch (run 34734308703). Tokenomics unchanged.
...
- GitHub Actions is three-tier (Refs #69): fast PR (`test:lib` + targeted Foundry when Solidity changes), full merge-candidate (`ready_for_review`, label `ci-full`, or `workflow_dispatch`), one main post-merge path. Feature-branch `push` no longer duplicates `pull_request`. `keeper-lease-pg` is folded into `postgres-ms-timestamps`. #35 production browser + wallet E2E is the full-only job `e2e-release-gate` on this same file — not a second `push`+`pull_request` workflow. Required #37 page-budget job is always-on (every PR, including drafts) and is required by `ci-ok` (skipped ≠ pass). File stays in `pnpm test:lib`. No nightly schedule. Operator inventory: `/docs/ci`. Tokenomics unchanged.
...
- Trusted geo / jurisdiction policy layer (issue **#63** — core accepted on merged #67 `e712617` / [`34731788819`](https://github.com/solarcurvey/reactor/actions/runs/34731788819); **reopened** until #65 user-visible restricted state via PR #75): server `evaluateRequestGeo` returns ALLOW / DENY / UNKNOWN with reason codes. Production country/region only from a verified edge HMAC (`GEO_EDGE_SECRET`). Browser `CF-IPCountry` / `X-Country` headers are ignored. Versioned deny revision `geo-policy-us-comprehensive.v1.json` **revision 3** records OFAC program source + effective date. Comprehensive jurisdictions are `CU` / `IR` / `KP` only. **Syria (`SY`) is not blanket-denied** — OFAC ended the comprehensive Syria program effective 2025-07-01 (E.O. 14312; 31 CFR part 542 removed). Remaining Syrian restrictions are list-based/targeted (#61 / #62). **Donetsk / Luhansk oblasts are not blanket-denied** — [OFAC FAQ 1009](https://ofac.treasury.gov/faqs/1009): E.O. 14065 does not block the entire oblasts. Trusted `UA-14` / `UA-09` or oblast-level names → `UNKNOWN_REGION_METADATA_UNAVAILABLE`. DENY only on a signed covered-region claim (`UA-DPR` / `UA-LPR` or `DNR`/`DPR`/`LNR`/`LPR` / People's Republic names). LOCAL fixture `FX`/`FY` cannot load production ISOs. VPN/Tor is best-effort only. HTTP write enforcement is #62. **Not** address screening (#61), **not** a legal opinion or OFAC-compliance claim.
- Operator policy gate (Refs **#62 closed** after #68 `2002aed` / [`34733128955`](https://github.com/solarcurvey/reactor/actions/runs/34733128955); parent **#60** stays open): one shared server decision (`allow` / `deny` / `unavailable`) on REACTOR-operated write/authorization paths — `POST /launch/admit`, `POST /launch/authorize`, isolated launch signer, `POST /quote`, `POST /upload`, Next `POST /api/launch-pricing`. Subject is the recovered EIP-191 signer of `GET /operator-policy/challenge` — not `body.wallet` / `x-reactor-wallet`. Fail closed on blocked recovered wallet, blocked geo, missing/invalid proof, or stale/unavailable required policy. Browser `sanctionsClear` / country / IP / claimed-wallet flags cannot override. Quote `recipient` and launch `creator` are rebound to the recovered signer. `GET /operator-policy/status` is the official minimized decision read for #65 / PR #75. Production HTTP matrix (`pnpm test:operator-policy-http`, full-only job `operator-policy-http`) starts the real indexer and production Next. Address screen binds merged **#66** `indexerSanctionsStore().screen`; trusted geo binds merged **#67** `evaluateRequestGeo`. `GET /sanctions/screen` stays the ungated lookup. Public GET market/docs paths stay readable. Does not block direct onchain contract use. Not a legal/OFAC-compliance opinion. Production without both official plugins fail-closes.
- Restricted-access UX (Refs **#65**, parent **#60**; **#63** stays open until this lands): dedicated `/restricted` state, site banner, and disabled Confirm / Launch / Quote / bid / claim CTAs when the hosted operator-policy decision is deny or unavailable. Neutral copy for account, location, and temporary unavailability (including #64 stale/missing official-list / 7-day SLA). Browser never receives raw IP, screening-entry metadata, list UIDs, or dataset hashes. No VPN/bypass guidance. Exact-list matching is not hop analytics. Public market/docs reads stay available. Does not censor permissionless chain reads or pause immutable contracts. Bound to merged **#68** `GET /operator-policy/status` (same `evaluateOperatorPolicy` as write gates) + `GET /operator-policy/challenge` signing helper, on `main` that includes merged **#70** and squash-merged **#44** (`b190e86`). Pending-proof reasons stay Launch Instant in the UI; the write gate remains authoritative. Production `next build`/`next start` matrix: blocked wallet / geo / stale / allow on desktop and 390px (`e2e/restricted-prod.spec.ts` via `test:web-security`). #35 mock `/operator-policy/status` uses the official public ALLOW view so #44 release journeys do not fail-close. Production visual/a11y/reflow gate covers `/restricted` and a denied launch/token (`e2e/a11y.spec.ts`: axe + `assertNoSubAaMutedText` + 320px / 200% reflow). `/restricted` muted headings and disclosure use `text-zinc-400` (do not weaken the #49 AA floor). `/restricted` reads `?kind=` on the server and does not call `useSearchParams()` (avoids React #418 hydration text/HTML under production `next start`). Not a legal/OFAC-compliance opinion.
...
- Sanctions ops (#64): 7-day official-list SLA, atomic last-known-good refresh (startup + 6h), health names exact dataset + policy versions, minimized policy-decision audit, `sanctions_*` alerts, operator runbook linked from incident response. Stale/missing data fails protected writes (`UNAVAILABLE_DATASET_STALE`). No automated override on complaint. Gated/logged identity is the #62 recovered EIP-191 wallet only — `body.wallet` / `x-reactor-wallet` are ignored. Fixture refresh is LOCAL/test-only; PROD/PRODUCTION/STAGING/TESTNET require the official #61 source. Same-address refresh persists a new #61-style source-generation (restart-safe `retrievedAt`). After #66, ops refresh binds the shared `#61` store and persists freshness under `SANCTIONS_DATA_DIR/ops`. LOCAL official-store bind loads pinned `#61` fixtures (no live OFAC unless `SANCTIONS_NETWORK=1`) so the `#62` HTTP matrix sees current freshness. Refs #64.
- Production UI release gate (issue #35): Playwright against `next build`/`next start`, Chromium/Firefox/WebKit plus iPhone-class and narrow-Android viewports, deterministic EIP-1193 wallet (Anvil #0/#1 addresses only — no mainnet keys). BUY/SELL/nested BUY+SELL (`UserRouteExecutor` calldata)/bonding/graduated/launch/rewards plus wrong-chain and user-reject. Chromium also loads a MetaMask/Rabby-style MV3 extension (MAIN-world inject, CJS-safe harness — no `import.meta`) and covers lock/switch/disconnect, revert (receipt `status`), allowance, quote TTL, dropped tx, double-submit, and Dev Buy. Suite is single-worker against a shared mock. Unexpected `console.error` / `pageerror` fail teardown via a shared fixture (`e2e/harness/console-gate.ts`; documented allowlist: Chromium HTTP 503 on `/api/launch-pricing` during Dev Buy authorize-down, Next.js RSC prefetch fallback on iPhone WebKit / Firefox, and WebKit EventSource `/stream` access-control only — indexed GETs echo Origin on the mock JSON response). Header `WalletButton` is the only connect/Account control; `/wallet` is a status card. Ticket/launch phase lines use visible `text-zinc-400` (same AA muted floor as #49). `pnpm docs:check` rejects leftover conflict markers. CI: `.github/workflows/ci.yml` full-only job `e2e-release-gate` (`xvfb-run`). No separate `e2e-release.yml`. Issue stays open until merge + post-merge verify. Tokenomics **unchanged**.
