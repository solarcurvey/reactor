...
- Live UI: bottom-right toasts for **confirmed** CORE `BuybackExecuted` / `COREBurned` and Top-10 `Top10Buy` after indexer SSE commit. First-session `hello.head` skips history; reconnect `?after=` delivers missed live events once. Dedupe is `(chainId, tx, logIndex, eventKind)` and survives dismiss. Hover/focus pauses auto-dismiss; safe-area insets; reduced-motion skips enter animation. Visible CI gate `live-toasts-ui` (`.github/workflows/live-toasts.yml`). Issue #38 stays open until post-merge verify. Economics unchanged.
