...
- Restricted-access UX (Refs **#65**, parent **#60**; **#63** stays open until this lands): dedicated `/restricted` state, site banner, and disabled Confirm / Launch / Quote / bid / claim CTAs when the hosted operator-policy decision is deny or unavailable. Neutral copy for account, location, and temporary unavailability. Browser never receives raw IP, screening-entry metadata, or list UIDs. No VPN/bypass guidance. Public market/docs reads stay available. Does not censor permissionless chain reads or pause immutable contracts. Bound to merged **#68** `GET /operator-policy/status` (same `evaluateOperatorPolicy` as write gates) + `GET /operator-policy/challenge` signing helper. Pending-proof reasons stay Launch Instant in the UI; the write gate remains authoritative. Not a legal/OFAC-compliance opinion.
