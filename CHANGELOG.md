...
- Operator policy gate (Refs **#62**, parent **#60**): one shared server decision (`allow` / `deny` / `unavailable`) on REACTOR-operated write/authorization paths — `POST /launch/admit`, `POST /launch/authorize`, isolated launch signer, `POST /quote`, `POST /upload`, Next `POST /api/launch-pricing`. Fail closed on blocked wallet, blocked geo, or stale/unavailable required policy. Browser `sanctionsClear` / country / IP flags cannot override. Public GET market/docs paths stay readable. Does not block direct onchain contract use. Not a legal/OFAC-compliance opinion. #61 address screening and #63 trusted geo plug in when present; production without those plugins fail-closes.
