...
- Consensus applies documented staleness (120s), deviation (150 bps), optional Arc executable-market sanity (400 bps), and `minSources`. Accepted and rejected observations persist (`kind=observation|consensus`). Schema **v10** adds `external_price_marks.kind` after the #27/#23 train (main v8 journal; v9 reserved for `tokens.current_supply` on PR #23). A real post-#27 v8 database `ALTER`s + backfills `source IN ('consensus','fused','fail','missing')`. v10 is applied by migration id existence so a later v9 insert still works. Arc sanity reads the executable mark from the verified `route_venues` row (`last_price_quote_x18` / JSON `data`), not a synthetic REACTOR `markets` row.
