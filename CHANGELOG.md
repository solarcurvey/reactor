...
- Trusted geo / jurisdiction policy layer (issue **#63**): server `evaluateRequestGeo` returns ALLOW / DENY / UNKNOWN with reason codes. Production country/region only from a verified edge HMAC (`GEO_EDGE_SECRET`). Browser `CF-IPCountry` / `X-Country` headers are ignored. Versioned deny revision `geo-policy-us-comprehensive.v1.json` **revision 2** records OFAC program source + effective date. Comprehensive jurisdictions are `CU` / `IR` / `KP` only. **Syria (`SY`) is not blanket-denied** — OFAC ended the comprehensive Syria program effective 2025-07-01 (E.O. 14312; 31 CFR part 542 removed). Remaining Syrian restrictions are list-based/targeted (#61 / #62). LOCAL fixture `FX`/`FY` cannot load production ISOs. VPN/Tor is best-effort only. **Not** an HTTP gate (#62), **not** address screening (#61), **not** a legal opinion or OFAC-compliance claim.
