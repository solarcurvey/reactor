...
npx --yes tsx apps/web/src/lib/operator-policy-status.test.ts
...
- Edge file `e2e/release/edge.spec.ts`: disconnect/reconnect, account/chain change mid-flow, locked wallet, insufficient funds/allowance, revert, dropped tx, quote TTL, quote expiry while the wallet prompt is held, double-submit lock, full `idle → quoting → approval/signature → submitted/pending → confirmed`, Dev Buy happy + authorize-down failure. Chain-change asserts Confirm (`trade-confirm`) and Claim (`rewards-claim`) by test id — both share the “Wrong network” label after #65 `useOperatedWrites`, so a `getByRole` name locator is not unique.
...
| 58 | Visual / a11y / failure-injection gate; CI fails on unexplained screenshot, serious axe diffs, color-contrast, leftover `text-zinc-500|600|700`, or unexpected console/pageerror. Includes `/restricted` (allowed-user QA mock) and denied operator-policy launch/token (`e2e/a11y.spec.ts`: axe + `assertNoSubAaMutedText` + 320px / 200% reflow, one page per denied-reflow test). Operator-policy UX stays on the SSR pending tree until after mount so a fast mocked `/status` cannot React-418. | `e2e/visual.spec.ts`, `e2e/states.spec.ts`, `e2e/a11y.spec.ts`, `e2e/failures.spec.ts`, `qa-inject.test.ts`, `e2e/contrast.test.ts`, `e2e/console-gate.test.ts` |
...
| 63 | AutomationGateway signed `MaintenanceJob`s (#51): typed actions only (no generic `target.call`); EIP-712 bind; first valid consume + second relayer `Replay`; relayers have no decision authority. CRE simulate is optional interoperability. Production KMS / dual-relay is #83. Issue #51 stays open. | `AutomationGateway.t.sol`, `AutomationGatewayAuth.t.sol`, `MaintenanceFailover.t.sol`, `packages/reactor/src/maintenance-job.test.ts` |
