...
| 62 | Restricted-access UX (#65): `/restricted` + disabled write CTAs on deny/unavailable; public reads remain; no IP/screening leak; no VPN guidance; official `GET /operator-policy/status` + challenge signing helper + write gate; pending-proof stays Launch Instant. Issue stays open. | `operator-policy-ux.test.ts`, `operator-policy-status.test.ts`, `e2e/restricted.spec.ts`, `e2e/restricted-prod.spec.ts` |
