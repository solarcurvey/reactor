...
| 60 | Operator policy gate (#62): recovered wallet proof; sign-as-BLOCKED + claim CLEAR still denies; blocked geo / allow / stale dataset / missing proof / public GET reads / denial before signer/upload/tx payload. Official `#66` `indexerSanctionsStore().screen` + `#67` `evaluateRequestGeo` bind. | `packages/reactor/src/sanctions-policy.test.ts`, `packages/reactor/src/wallet-proof.test.ts`, `apps/indexer/src/operator-policy.test.ts`, `apps/web/src/lib/operator-policy-bff.test.ts` |
