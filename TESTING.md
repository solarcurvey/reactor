...
| 61 | Sanctions freshness SLA, last-known-good refresh, stale protected writes, health versions, audit redaction, failure-injection alerts; recovered-identity-only gate (merged #62 `operator-policy.ts` / wallet-proof); LOCAL/test-only fixture fallback (#64) | `packages/reactor/src/sanctions-ops.test.ts`, `packages/reactor/src/sanctions-audit.test.ts`, `apps/indexer/src/sanctions-ops.test.ts` |
