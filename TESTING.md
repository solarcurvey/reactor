...
| 60 | Operator policy gate (#62): blocked wallet / blocked geo / allow / stale dataset / spoofed client clear flags / public GET reads / denial before signer/upload/tx payload. Real indexer-shaped + Next BFF routes. Official `#66` `indexerSanctionsStore().screen` + `#67` `evaluateRequestGeo` bind when present. | `packages/reactor/src/sanctions-policy.test.ts`, `apps/indexer/src/operator-policy.test.ts`, `apps/web/src/lib/operator-policy-bff.test.ts` |
