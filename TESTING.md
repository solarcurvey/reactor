...
- Shared console/pageerror gate (`e2e/harness/console-gate.ts`) on every release-gate page (EIP-1193 `wallet.ts` and MV3 `extension.ts` fixtures). Records `page.on('console')` error-level messages and `page.on('pageerror')`. Documented allowlist only: Chromium HTTP 503 on `/api/launch-pricing` (Dev Buy authorize-down, fail-closed); Next.js RSC prefetch fallback on iPhone WebKit / Firefox home (`?_rsc=` access-control `pageerror` on WebKit; full navigation still works); WebKit mock `:18448` access-control on named public GET/SSE (`/stream`, `/markets`, `/quote-assets`, `/operator-policy/{challenge|status}` — optional `Fetch API` prefix; not a generic `:18448` or 404 allowlist). Header `WalletButton` is the only connect/Account control; `/wallet` is a status card. Teardown fails with the captured diagnostics. Trace / screenshot / network stay retain-on-failure.
