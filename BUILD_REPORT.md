...
| Review shots | Refreshed for the visible AA `text-zinc-400` phase line and mobile Quote/Confirm stack. E2E wallet helpers scope to `banner` because `/wallet` also mounts `WalletButton`. `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. **Refs #35 / Refs #15 only.** |
