...
| Review shots | Regenerated for the visible AA phase line and mobile Quote/Confirm stack. Discover board stays on the #49 fixture set (Ready is mock-only). `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. **Refs #35 / Refs #15 only.** |
