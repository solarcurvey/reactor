...
| Tests | Production four-state AC closed on this PR. a11y/contrast: `/restricted` + denied launch/token in `e2e/a11y.spec.ts` (axe + `assertNoSubAaMutedText` + 320px / 200% reflow). `/restricted` muted copy is `text-zinc-400` (do not weaken the #49 assertion). Local counts recorded after verify. |
