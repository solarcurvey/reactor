...
| Unexpected console.error / pageerror fail the suite | **Yes** | Shared `attachConsoleGate` on EIP-1193 and extension page fixtures. Records error-level `console` + `pageerror`. Documented allowlist: Chromium HTTP 503 on `/api/launch-pricing` (Dev Buy authorize-down); Next.js RSC prefetch fallback (iPhone WebKit + Firefox) / WebKit `?_rsc=` access-control; WebKit mock `:18448` access-control on named public GET/SSE (`/stream`, `/markets`, `/quote-assets`, `/operator-policy/{challenge|status}` — not a generic `:18448` or 404 allowlist; iPhone home reports `/markets?limit=80` the same way as `/stream`). Mock `/stream` echoes the request `Origin`. Teardown throws captured diagnostics. Trace/screenshot/network stay retain-on-failure. |
