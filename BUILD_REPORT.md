...
| Review shots | **Not regenerated** (fixture `curve` / ready token only; no chrome change) | Mock indexer serves `#50` `GET /quote-assets`, `GET /markets/:token`, and `GET /page/token/:token`, plus `#68` `GET /operator-policy/challenge` and `GET /operator-policy/status` so quote/launch/upload can attach a wallet proof. `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. |
| Mainnet | **Blocked** |
