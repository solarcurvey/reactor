...
| Tests | Local verify after a11y/contrast: `pnpm test:lib` green; `pnpm test:restricted` **7/7**; `pnpm test:web-security` **13/13** (2 XSS + 11 production restricted-prod); `pnpm test:web:qa` **138/138** (was 131; +`/restricted` axe + denied launch/token axe + 320px / 200% denied reflow). `/restricted` muted copy is `text-zinc-400` (do not weaken `assertNoSubAaMutedText`). Production four-state AC closed on this PR. |
