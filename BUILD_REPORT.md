...
| Proof | Rebased onto `main` `35552f6` (#79 after #68 / #67 / #66 / #49 / #42 / #50 / #58). Kept #79 accepted #17 / #42 evidence (`11fdadb` / `34727535121`, `80d3cac` / `34727638255`). Kept #68 operator-policy / wallet-proof. Kept #67 geo-policy tests in `test:lib`. Kept #66 `@reactor/sanctions` fixtures in `test:lib`. Kept #49 `web-qa` (visual / a11y / failure-injection). Kept #42 `docs-links` / Playwright `web` / Safe genesis / solc prefetch. Kept #50 indexed search / `useSwapSeries` / `readTicketWallet` / always-on `page-budget`. Live quote default stays **30s** (`?? 30_000`); E2E short TTL is `NEXT_PUBLIC_QUOTE_TTL_MS` only. Mock indexer serves `#50` `GET /quote-assets`, `GET /markets/:token`, and `GET /page/token/:token` so Chromium does not fail the console-gate on those 404s. REVIEW_FIXTURES merge restores `curve` / `ready` / fixture quote on `/page/token` rows so bonding hits InstantCurve and graduate stays visible. Indexed bonding/ready rows bind official `InstantCurve`; REVIEW_FIXTURES merge restores `curve` / `ready` / fixture quote on `/page/token` so BUY/graduate do not fall through to the router. **Refs #35 / Refs #15 only.** |
| Review shots | **Not regenerated** (fixture `curve` / ready token only; no chrome change) | `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. #35 stays open until post-merge. **Refs #35 / Refs #15 only.** || Review shots | **Not regenerated** (fixture `curve` / ready token only; no chrome change) | Phase testids are `sr-only` (no `text-zinc-500`). Ready graduate token stays on the E2E mock only — not an extra Discover fixture card. Rewards wallet copy waits until mount so SSR matches the first client paint. Phase testids are `sr-only` (no `text-zinc-500`). Ticket/launch phase lines use visible `text-zinc-400` (same AA muted floor as #49 — do not hide the line and do not weaken `assertNoSubAaMutedText`). **Refs #35 / Refs #15 only.** |
| Review shots | Regenerated for the visible AA phase line and mobile Quote/Confirm stack. Discover board stays on the #49 fixture set (Ready is mock-only). **Refs #35 / Refs #15 only.** |
| Review shots | Need refresh for the visible AA `text-zinc-400` phase line and mobile Quote/Confirm stack. Disconnect stays in the Account modal. Ticket/launch phase testids are `sr-only` so Discover/launch/trade goldens stay on #49 chrome (E2E still reads `data-phase`). Disconnect stays in the #49 Account modal; E2E waits for `wallet-menu-trigger`. **Refs #35 / Refs #15 only.** |
| Review shots | **Not regenerated.** Phase testids are `sr-only`. #35 stays open until post-merge. Rewards wallet copy waits until mount. WalletButton keeps the SSR Connect tree until hydrate so a second `page.goto` after EIP-1193 connect cannot React-418. **Refs #35 / Refs #15 only.** |
| Review shots | Refreshed for the visible AA `text-zinc-400` phase line and mobile Quote/Confirm stack. E2E wallet helpers scope to `banner` because `/wallet` also mounts `WalletButton`. Disconnect stays in the #49 Account modal. `/wallet` is a status card only — the header `WalletButton` is the single connect/account control (`wallet-menu-trigger` is unique). `/wallet` no longer duplicates the header wallet control. Mock indexer serves `#50` `GET /quote-assets`, `GET /markets/:token`, and `GET /page/token/:token`. **Refs #35 / Refs #15 only.** |
| Review shots | Refreshed for the visible AA `text-zinc-400` phase line (including quote-413 / wallet-revert / tx-reverted 1440). | `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. **Refs #35 / Refs #15 only.** |
| Review shots | **Not regenerated** (fixture `curve` / ready token only; no chrome change) | Mock indexer serves `#50` `GET /quote-assets`, `GET /markets/:token`, and `GET /page/token/:token`, plus `#68` `GET /operator-policy/challenge` and `GET /operator-policy/status` so quote/launch/upload can attach a wallet proof. Mock indexer serves `#50` `GET /quote-assets`, `GET /markets/:token`, and `GET /page/token/:token`, plus `#68` `GET /operator-policy/challenge` and `GET /operator-policy/status` so quote/launch/upload can attach a wallet proof. EIP-1193 `rejectTx` is Confirm-buy / `eth_sendTransaction` only (challenge `personal_sign` auto-signs). Extension Quote Confirm covers the proof, then Confirm covers the trade. WebKit console-gate allowlists named mock GET/SSE (`/stream`, `/markets`, `/quote-assets`, `/operator-policy/{challenge|status}`) — not generic 404s. WebKit console-gate stays pinned to exact `/127.0.0.1:18448/stream` pageerror only. Home+launch waits for mock `GET /markets` before leaving `/` so iPhone WebKit does not abort that fetch and mis-report it as CORS. `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. |
| Mainnet | **Blocked** |
...
| Unexpected console.error / pageerror fail the suite | **Yes** | Shared `attachConsoleGate` on EIP-1193 and extension page fixtures. Records error-level `console` + `pageerror`. Documented allowlist: Chromium HTTP 503 on `/api/launch-pricing` (Dev Buy authorize-down); Next.js RSC prefetch fallback (iPhone WebKit + Firefox) / WebKit `?_rsc=` access-control; WebKit mock EventSource `/stream` access-control (exact `/127.0.0.1:18448/stream` text; other `:18448` paths stay unexpected). Home+launch waits for mock `GET /markets` before `/launch` so iPhone does not abort-as-CORS. Mock `/stream` echoes the request `Origin`. Teardown throws captured diagnostics. Trace/screenshot/network stay retain-on-failure. |
