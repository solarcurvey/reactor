...
| Tests | Local verify after a11y/contrast: `pnpm test:lib` green; `pnpm test:restricted` **7/7**; `pnpm test:web-security` **13/13**; `pnpm test:web:qa` **138/138** on `311957f`. Ready-for-review rerun [`34738474644`](https://github.com/solarcurvey/reactor/actions/runs/34738474644) failed `web-qa` on React 418 in denied `/restricted` a11y (Suspense/`useSearchParams` + policy fetch during hydrate). `/restricted` no longer uses `useSearchParams`; provider refresh waits until mount. `assertNoSubAaMutedText` unchanged. |
...
| --- | --- | --- |
| Dedicated `/restricted` state | **Yes** | `apps/web/src/app/restricted/page.tsx` + amber banner |
