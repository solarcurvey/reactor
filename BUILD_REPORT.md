...
| Exact-commit desktop/mobile visual gate | **Yes (gated)** | `docs/visual-accept.json` hash `d51bb17f…` after the branded replay. Discover/token/quote/CORE `web-qa` Chromium goldens recaptured on this tip. Launch/Fair/trade/THE REACTOR/`/restricted` goldens were not overwritten. `e2e/visual-gate.spec.ts` designed error/empty shots use `?inject=indexer` and `?inject=empty` so they do not treat `REVIEW_FIXTURES` fallback as a live indexer. |
