...
| Unexpected console.error / pageerror fail the suite | **Yes** | Shared `attachConsoleGate` on EIP-1193 and extension page fixtures. Records error-level `console` + `pageerror`. Documented allowlist: Chromium HTTP 503 on `/api/launch-pricing` (Dev Buy authorize-down); Next.js RSC prefetch fallback (iPhone WebKit + Firefox) / WebKit `?_rsc=` access-control; WebKit mock EventSource `/stream` access-control (exact `/127.0.0.1:18448/stream` text; other `:18448` paths stay unexpected). Mock `/stream` echoes the request `Origin`. Teardown throws captured diagnostics. Trace/screenshot/network stay retain-on-failure. |
