...
| Review shots | Need refresh for the visible AA `text-zinc-400` phase line and mobile Quote/Confirm stack. Disconnect stays in the Account modal. `ci-ok` requires `e2e-release-gate` **and** `web-qa` **and** `page-budget`. **Refs #35 / Refs #15 only.** |
