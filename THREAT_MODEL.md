...
14. **Stale / missing official-list data** — operated writes fail closed (`UNAVAILABLE_DATASET_STALE` / `MISSING`). A bad refresh keeps last-known-good. Gated identity is the #62 recovered EIP-191 wallet, not a claimed body/header. Fixture refresh is LOCAL/test-only. Residual: the 7-day SLA is offchain; public contracts remain callable. No automated delist on complaint. See `docs/sanctions-ops.md`.
...
- Browser telemetry (`POST /api/telemetry`) is capped and re-redacted. A compromised page can still *attempt* to exfiltrate through a configured Sentry DSN; residual-secret rejection and key/signature redaction are the control. Telemetry is not custody. See `/docs/observability`.
