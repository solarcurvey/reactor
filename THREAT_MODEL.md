...
14. **Stale / missing official-list data** — operated writes fail closed (`UNAVAILABLE_DATASET_STALE` / `MISSING`). A bad refresh keeps last-known-good. Residual: the 7-day SLA is offchain; public contracts remain callable. No automated delist on complaint. See `docs/sanctions-ops.md`.
