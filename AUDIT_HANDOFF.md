...
| Operator policy gate (P1, #62) | Shared allow/deny/unavailable on REACTOR-operated write/auth paths. Binds `#66` `indexerSanctionsStore().screen` and `#67` `evaluateRequestGeo`. Fail closed. Recovered-wallet proof + `GET /operator-policy/status`. No client-flag override. Does not claim onchain blocking. | `sanctions-policy.test.ts`, `wallet-proof.test.ts`, `operator-policy.test.ts`, `operator-policy-bff.test.ts`, `/docs/operator-policy` |
