...
**This amendment (exact official-list screening, Refs #61 / parent #60):** `@reactor/sanctions` parses pinned OFAC-shaped SDN/consolidated XML fixtures and can refresh official Treasury HTTPS lists atomically. `screen()` / `GET /sanctions/screen` returns `blocked` | `clear` | `unavailable` plus dataset version — never a boolean that treats errors as clear. **Not legal/OFAC compliance. No hop/exposure attribution. Not a launch/trade policy gate** (later #60 children). Architecture and tokenomics unchanged. Re-audit parser canonicalization, atomic activate, and freshness fail-closed. Do not treat this as a protocol change.
...
| Exact official-list screening (#61) | OFAC/Treasury HTTPS XML only. EVM 20-byte identity. Atomic refresh. `blocked` / `clear` / `unavailable`. No compliance claim. No #60 policy gate. | `packages/sanctions/src/*.test.ts`, `apps/indexer/src/sanctions-api.test.ts`, `SANCTIONS.md`, `/docs/sanctions` |
