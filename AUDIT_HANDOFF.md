...
| Repo publicization / public-fork Actions (P1 ops) | Visibility stays private until founder instruction. History rewrite of personal trailers **done** (noreply). AC1 = advertised refs only; residual dangling SHAs accepted (Support purge/GC not required). Workflow `contents: read` + `persist-credentials: false`; no `pull_request_target`. Compatible with #69. Re-scan secrets before any visibility change. | `docs/publicization.md`, `scripts/ci-public-harden.test.ts`, `.github/workflows/*.yml` |
