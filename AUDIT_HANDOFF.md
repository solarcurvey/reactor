...
| Exact official-list screening (#61) | OFAC/Treasury HTTPS XML only (SDN + Consolidated). EVM 20-byte identity. Atomic refresh with 85% completeness floor. `blocked` / `clear` / `unavailable`. No compliance claim. No #60 policy gate. | `packages/sanctions/src/*.test.ts`, `apps/indexer/src/sanctions-api.test.ts`, `SANCTIONS.md`, `/docs/sanctions` |
