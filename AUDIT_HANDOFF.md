...
| Live buy+burn toasts | SSE after persist commit. First `hello.head` skips history; reconnect `?after=` must not raise cutoff. Dedupe `(chainId, tx, logIndex, eventKind)` survives dismiss. Hover/focus pause; safe-area; reduced-motion. #38 stays open until merge + post-merge verify. | `live-toasts.test.ts`, `live-sse.test.ts`, `sse.test.ts`, `e2e/live-toasts.spec.ts` |
