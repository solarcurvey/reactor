...
**In-app handbook:** `/docs` is the GitBook-style reading path (audience, protocol, launch, builders, reference). It restates frozen V1 economics; it does not invent new fees or knobs. Root files (`ECONOMICS.md`, `ARCHITECTURE.md`, `THREAT_MODEL.md`, …) remain the auditor-facing sources. `llms.txt` mirrors the handbook index.
