...
| Live buy+burn toasts | SSE after persist commit. `hello.head` skips replay. UI toasts only CORE `BuybackExecuted`/`COREBurned` and `Top10Buy`. Not SelfBurn, not epoch submit, not mempool. | `live-toasts.test.ts`, `live-sse.test.ts`, `sse.test.ts` |
