...
**Protocol release:** `0.4.0` (`v0.4.0`, `docs/version.json`). **Factory version:** V1 (`FACTORY_VERSION = 1`) — immutable, not the protocol semver.

**This amendment (#83 reconcile on draft #54):** Founder selected AWS KMS + dual managed relays (**#83**) as the production autonomous execution path. That path does **not** depend on Chainlink CRE approval/support. Authenticated `cre workflow simulate` is **optional interoperability evidence** — not a #54 merge blocker and not the #51 production closer. `AutomationGateway` stays typed-only (no generic `target.call`); EIP-712 payload / chain / snapshot / window / replay binding is unchanged. Local Anvil dual-relayer + autonomous failover remain Gateway evidence, not #83 KMS/testnet proof. Issue **#83 stays open**. Issue **#51 stays open** through the real #83 autonomous testnet/failover proof **and** post-deploy `Guardian.keeper == AutomationGateway`. #54 becomes merge-ready only after this mapping plus exact-head full regression.

**This amendment (AutomationGateway #51 draft for re-audit on #48 `922f909`):** Draft PR #54. `AutomationGateway` is `ReactorGuardian.keeper`. Typed EIP-712 `MaintenanceJob`s only — no generic `target.call`. Relayers have no decision authority. Dual-relayer proofs: Foundry via `foundry-targeted` / `solidity + size-guard` `forge test`; rehearsal scripts on the full/main solidity job. Signed-job CRE courier + HTTP payload are in-repo (`ops/cre/maintenance-courier`, `signed-job-http-payload.json`). Public CI step **Verify committed CRE evidence / tenant-blocker (not a CRE simulate)** runs `cre-workflow-simulate.ts --verify` — pin-check only; it is not a Workflow Simulation Result. Official `cre workflow build` (CLI v1.33.0, no tenant) compiled that courier to WASM (`ops/cre/simulation/cre-workflow-build.json`). Authenticated `cre workflow simulate` still requires a CRE tenant (`cre login` or `CRE_API_KEY`) and remains unrun (`ops/cre/simulation/cre-tenant-blocker.json` `closed:false`). After **#83**, that simulate is **optional interoperability**, not a merge/production blocker. Not a live CRE DON on EIP-155 1883 and not 5042. Merged #80 Industrial Forge (Direction C) production chrome/assets stay. After the #48 handbook rebase, generated `llms.txt` twins still said protocol `0.3.4` while `docs/version.json` is **0.4.0**; `pnpm docs:gen` rewrote only those twins. Playwright `e2e/protocol-version.ts` reads `docs/version.json` via cwd fallbacks (CJS-safe; do not import `release-identity` / `import.meta`). **#39 is open/reopened** — merged #46 landed repo CI; close still requires the real configured telemetry-provider symbolication proof. Issue **#51 stays open** through **#83** autonomous testnet/failover proof **and** post-deploy `Guardian.keeper == AutomationGateway`.

**This amendment (frontend ops / #39, merged #46 / `789eb5c`):** the public Next app reports **redacted** failure telemetry (API, RPC, wallet, quote, SSE, tx, media, UI, simulation) tagged with `reactor@{protocol}+{git SHA}` **and** exact `reactorEnv` / `chainId` / `chainName` / `buildTimestamp`. Core-page Web Vitals. Operator paging runbook (render / API / RPC / quote / SSE / simulation); expected wallet `4001` never pages. User-visible errors show `ref {traceId} · chain {chainId}` (`x-request-id`). Optional Sentry DSN. No private keys, launch signatures, Turnstile tokens, or cookies. Source maps are generated, archived, and **not** served in production; first-party symbolication resolves a generated production stack to original source. Configured-DSN staging vendor proof (`obs/vendor-proof.test.ts`) POSTs that symbolicated event to a valid Sentry store mock and asserts original frames + release/env/chain tags. Exact-head CI job **`obs-ui`** is folded into `.github/workflows/ci.yml` (full / main / `ci-full`; production `next start`; public-fork harden). #75 `/restricted` + `policy.ensureProof` stay. #44 `e2e-release-gate` + #70 sanctions-ops docs/nav + `/ops` dataset card stay. #49 `web-qa` + #42 leftover #17 gates (`docs:links`, Playwright `web`, Safe genesis in `test:lib`) stay. `POST /api/telemetry` 429 is intended ingest backpressure (40/10s/IP in genuine production; review-fixture / `REACTOR_TELEMETRY_RELAXED` artifacts raise the cap); Chromium `Failed to load resource` on that URL is not a `web-qa` page diagnostic. Quote `?inject=quote-429` stays fail-visible. Issue **#39 stays open** until the real configured telemetry-provider symbolication proof. Do not restore protocol **0.3.4** on this draft. Economics and hook/Factory surface are unchanged. Not an oracle. Not mainnet.
...
**This amendment (handbook, Refs #14, squash-merged #48 / `922f909`):** landed on `origin/main`. Handbook preserves Forge chrome (`rx-kicker` / `rx-link`) additively with body search, badge matrix, and nav blurbs. Handbook muted copy uses the approved AA floor (`text-zinc-400`); leftover `text-zinc-500` on `/docs` failed `web-qa` `assertNoSubAaMutedText` and is raised, not the assertion. The four #14 AC surfaces landed with the squash. This draft keeps protocol **0.4.0** (do not restore handbook `0.3.4`). Issue **#14** close is a founder decision. Architecture and tokenomics unchanged.

**This amendment (Industrial Forge brand, Refs #55):** founder-locked Direction C is the production visual identity — tokens, vessel mark, wordmark, favicon/OG/app icons, product chrome, and voice. Landed on main `4915f3e` after **#80**. After #81, docs-only/trivial ready PRs stay on the cheap `ci-decide` path; `ci-full` still launches `web-qa` / `obs-ui` / the rest of the full gate. After #75, `/restricted` and deny banners use the same system: heat `rx-kicker`, 0–4px radius, no cyan; amber stays the semantic deny warning. After #46, error boundaries keep release SHA + `traceId` and use heat kickers (no leftover cyan/pills). **#55 closed/completed** on `main` after the #48 post-merge handbook/visual proof. Issue #36 is closed (`ad7b457`, post-merge `34729758795`); `web-qa` remains required and must cover this approved-C state **plus #44 launch/trade phase and wallet chrome** (do not weaken that gate). No contract, fee, Factory, CORE, or routing change. See `/docs/brand`.
...
**This amendment (restricted-access UX, Refs #65 / parent #60, merged #75 / `4207356`):** public launchpad `/restricted` + disabled Confirm / Launch / Quote / bid / claim CTAs when the hosted operator-policy decision is deny or unavailable. Neutral account / location / temporary copy. Browser JSON is the minimized public view only (no raw IP, screening-entry metadata, list UIDs, dataset hashes). Official decision read is `GET /operator-policy/status` (`readOperatorPolicyStatus` / `evaluateOperatorPolicy`). `GET /operator-policy/challenge` is a signing helper only. Subject is the EIP-191 signer (`x-reactor-wallet-proof`). Official **#61 / #62 / #64** plugins are on `main` via **#66 / #68 / #70**. Disclosure matches merged #64: 7-day official-list SLA, stale/missing never treated as clear, last-known-good on the operator side, no complaint auto-override, exact-list matching is not hop analytics. Geo core is on `main` via **#67**; founder **reopened #63** until this user-visible restricted state lands. Production `next build`/`next start` matrix covers blocked wallet / geo / stale / allow on desktop and 390px plus the real #62 write-gate bypass. Production visual/a11y/reflow gate (`e2e/a11y.spec.ts`) covers `/restricted` and a denied launch/token (axe + `assertNoSubAaMutedText` + 320px / 200% reflow). `/restricted` muted copy is `text-zinc-400`. `/restricted` is a dynamic server page that passes `?kind=` into the client view (no `useSearchParams()` / text Suspense fallback) so production hydration matches. Does not pause or censor immutable public contracts. Architecture and tokenomics unchanged. **#65 and #63 stay open.**
...
**This amendment (AutomationGateway #51 rebase onto #81 `ff444cb`):** Draft PR #54 rebased onto `origin/main` after the docs-only CI path filter. Same PR / same branch. Mergeability restack only — do not chase a new `ci-full` matrix while CRE is blocked. `AutomationGateway` + typed EIP-712 `MaintenanceJob`s unchanged. #81 `ci-decide.sh` / cheap docs-only path, #46 `obs-ui` / redacted telemetry / error boundaries, and #75 `/restricted` + official `GET /operator-policy/status` kept as-is. Dual-relayer proofs stay reachable: Foundry via `foundry-targeted` / `solidity + size-guard`; rehearsal scripts on the full/main solidity job. No second `foundry.yml`. **CRE simulator AC is not closed.** Evidence: `ops/cre/simulation/cre-tenant-blocker.json`. Protocol **0.4.0** (do not restore #46 `0.3.4`). Playwright version pins read `docs/version.json` via CJS-safe `apps/web/e2e/protocol-version.ts` (Playwright compiles `apps/web` e2e as CJS; do not use `import.meta` in `*.spec.ts`). Issue **#51 stays open**. Architecture and tokenomics unchanged. Factory **V1** unchanged.

**This amendment (AutomationGateway #51 rebase onto #46 `789eb5c`):** Draft PR #54 rebased onto `origin/main` after production web observability. `AutomationGateway` + typed EIP-712 `MaintenanceJob`s unchanged. #46 `obs-ui` / redacted telemetry / error boundaries kept as-is. #75 `/restricted` + official `GET /operator-policy/status` kept as-is. Dual-relayer proofs stay reachable: Foundry via `foundry-targeted` / `solidity + size-guard`; rehearsal scripts on the full/main solidity job. No second `foundry.yml`. **CRE simulator AC is not closed.** Protocol **0.4.0** (do not restore #46 `0.3.4`). Full-tier Playwright version pins read `docs/version.json` via CJS-safe `apps/web/e2e/protocol-version.ts` (Playwright compiles `apps/web` e2e as CJS; do not use `import.meta` in `*.spec.ts`). Issue **#51 stays open**. Architecture and tokenomics unchanged. Factory **V1** unchanged.

**This amendment (AutomationGateway #51 rebase onto #75 `4207356`):** Draft PR #54 rebased onto `origin/main` after restricted-access UX. `AutomationGateway` + typed EIP-712 `MaintenanceJob`s unchanged. #75 `/restricted` + disabled write CTAs + official `GET /operator-policy/status` kept as-is. Dual-relayer proofs stay reachable: Foundry contracts via `foundry-targeted` / `solidity + size-guard` `forge test`; rehearsal scripts on the full/main solidity job. No second `foundry.yml` (would violate #69 `test:ci-cost`). **CRE simulator AC is not closed.** Issue **#51 stays open**. Architecture and tokenomics unchanged. Factory **V1** unchanged.

**This amendment (AutomationGateway #51 draft for re-audit on #44 `b190e86`):** Draft PR #54. `AutomationGateway` is `ReactorGuardian.keeper`. Typed EIP-712 `MaintenanceJob`s only — no generic `target.call`. Relayers have no decision authority. Dual-relayer proofs: Foundry via `foundry-targeted` / `solidity + size-guard` `forge test`; rehearsal scripts on the full/main solidity job. Signed-job CRE courier + HTTP payload are in-repo (`ops/cre/maintenance-courier`, `signed-job-http-payload.json`, `cre-workflow-simulate.ts --verify`). **CRE simulator AC is not closed.** Official `cre workflow simulate` requires a CRE tenant (`cre login` or `CRE_API_KEY`). This environment has neither; public CI has no secrets. The auth-blocked CLI transcript is **not** the #51 Arc Testnet CRE workflow/simulation PoC. See `ops/cre/simulation/cre-tenant-blocker.json`. Not a live CRE DON on EIP-155 1883 and not 5042. No second `foundry.yml` (would violate #69 `test:ci-cost`). **Gateway / job authorization is in external-audit scope.** This SHA is a **re-audit draft**, not an #18 freeze SHA. Issue **#51 stays open** until a successful authenticated simulate **and** post-merge `Guardian.keeper == AutomationGateway` verify. Merged #75 restricted-access UX (Refs #65; #65 / #63 stay open), merged #44 `e2e-release-gate` (Refs #35 / #15; #35 stays open), merged #70 sanctions-ops freshness (Refs #64), merged #79 #17/#42 evidence cites (`11fdadb` / `34727535121`, `80d3cac` / `34727638255`), merged #68 operator-policy (`operator-policy-http` required by `ci-ok`), merged #67 trusted geo/IP (`evaluateGeoPolicy`, edge HMAC), and merged #66 exact official-list OFAC screening (`@reactor/sanctions`, `GET /sanctions/screen`) are preserved. On main #61 / #62 / #36 / #37 are closed. **#17 stays open.** **#35 / #60 / #63 / #64 / #65 / #69 stay open.** #49 `web-qa`, #42 `docs:links` / Playwright `web` / Safe-genesis `test:lib`, #50 indexed aggregation / `page-budget`, and #58 ohlcv-chart tidy stay on main.

**This amendment (AutomationGateway #51 rebase onto #73 `300b7e5`):** Draft PR #54 rebased onto `main` after the CI cost cut. `AutomationGateway` + typed EIP-712 `MaintenanceJob`s unchanged. Dual-relayer proofs stay reachable: Foundry contracts via `foundry-targeted` / `solidity + size-guard` `forge test`; rehearsal scripts on the full/main solidity job. No second `foundry.yml` (would violate #69 `test:ci-cost`). Issue **#51 stays open**. Architecture and tokenomics unchanged.
...
**This amendment (CI cost residual, Refs #69, docs-only path filter):** Ordinary docs-only / trivial ready-for-review PRs no longer set `FULL=true`. `scripts/ci-decide.sh` + `scripts/ci-ok.sh` keep Solidity / Postgres / browser / `obs-ui` jobs off that cheap path, while `ci-full` / `workflow_dispatch` tier **full** / `push` to `main` still force every required job and reject skips. Deterministic proof: `pnpm test:ci-cost`. Do not weaken #15 / #17 / #18. **#69 stays open** until one green protected-main full run after merged #46 + this cost fix. Architecture and tokenomics unchanged.
...
**In-app handbook:** `/docs` is the GitBook-style reading path (audience, protocol, launch, builders, reference). It restates frozen V1 economics; it does not invent new fees or knobs. Root files (`ECONOMICS.md`, `ARCHITECTURE.md`, `THREAT_MODEL.md`, …) remain the auditor-facing sources. Sidebar search is full-text over page body. Version badges are protocol + Factory + API + SDK + source release. `llms.txt` is generated from `docs/version.json` + nav. `/docs/ci` is the #73 three-tier operator page. `/docs/qa` is the #49 visual / a11y / failure-injection gate. `/docs/sanctions` / `/docs/geo-policy` / `/docs/operator-policy` are the landed #66 / #67 / #68 pages. `pnpm docs:links` stays a local / full-tier sibling — do not restore `docs-sync.yml`. Base is `2002aed` (#68 after #67 / #66 / #49 / #42 / #50 / #58). No economics or Factory change.
...
| Keeper compromise | Gateway is `keeper`; job signer ≠ relayer in PROD; `KEEPER_MODEL.md`; modes DRY_RUN/LOCAL/ARC_TESTNET; 5042 hard-disabled; no key logs | `AutomationGateway.t.sol`, `AutomationGatewayAuth.t.sol`, `MaintenanceFailover.t.sol`, `GuardianP0.t.sol`, `keeper.ts`, `scripts/autonomous-relay-failover.ts` |
...
| `AutomationGateway` | `contracts/src/AutomationGateway.sol` | Designated `keeper`. EIP-712 `MaintenanceJob`. Typed vault jobs only. Relayers have no decision authority. Signed window `deadline-validAfter` ≤ 30m (`BadWindow`); not remaining time at execution. |
...
Keeper daemon (`apps/indexer/src/keeper.ts`) polls the API, writes a heartbeat, and signs a `submitEpoch` `MaintenanceJob` behind the `#6` lease fence. It does not broadcast as `Guardian.keeper` — the gateway is keeper; a distinct relayer submits the signed job. Independent watchdog (`apps/indexer/src/watchdog.ts`) fail-closes on stale / pause.

Onchain `submitEpoch` checks **structure only**. The gateway additionally binds ValuationService targets/weights + `valuationSnapshotHash` + `/pricing/health` in the signed job. Relayers cannot substitute ranking. Not a trustless oracle. CRE does not rank Top-10.
...
| Role | Power |
| --- | --- |
| Guardian | Pauses, replace Keeper / pricing signer / job signer, pause gateway, adapters, external quotes, one-time binds |
| Keeper (`AutomationGateway`) | Typed signed jobs: settle / submitEpoch / Top-10 buy+burn / roll / CORE execute / SelfBurn execute — hops/minOut/amount/snapshot bound |
| Relayer | Submit a valid job. Cannot change ranks, hops, or floors |
...
- Local Anvil 5042002: decision service signs jobs; a relayer submits them to `AutomationGateway`. Committed `deployments/local.json` is the pre-gateway demo dump (Keeper slot still Anvil #0). Run `Deploy.s.sol` for a live local Gateway — do not invent claimed addresses. Other chains refuse broadcast unless `KEEPER_PRIVATE_KEY` / job-signer material is set. Watchdog reads heartbeat **and** on-chain `epochFinalized`.
...
forge test --match-contract 'AutomationGateway|MaintenanceFailover' -vv
pnpm size:guard
...
3. CREATE2 hook bits
4. Job-signer compromise (ranks/routes/floors) or relayer liveness failure. Relayers cannot weaken a signed job. Dual-relayer consume/Replay: `MaintenanceFailover.t.sol` + `scripts/autonomous-relay-failover.ts`.
4b. Keeper sandwich despite signed `minTargetOut` (signer + quote-to-exec latency)
...
| AutomationGateway / job auth (#51 re-audit) | Typed jobs only; EIP-712 bind; `WrongAction` / `UnknownAction`; first consume wins. Draft — not an #18 freeze SHA. #51 stays open. `AutomationGateway.t.sol`, `AutomationGatewayAuth.t.sol`, `MaintenanceFailover.t.sol`, `docs/automation.md` |
