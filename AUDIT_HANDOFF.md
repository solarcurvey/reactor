...
**This software has not been audited.** Treat every contract as hostile-unreviewed. Do not deploy to Arc Mainnet (5042). No production / isolated-genesis claim from this PR. The 2026-09-12 #16 rehearsal Factory `0xB48D1B397834eBcccb8961041d827487097e0535` / Guardian `0x2CdF37541256749E5CF6ac5C806e0d23A685F224` on Arc Public Testnet (`5042002`) is **SUPERSEDED / non-PROD-isolated**: immutable `guardian()` = lost disposable `0xbeD4a2d496d280387FE65922fFbdf8C0f724bC6E`. Davis chose REDEPLOY. Isolated path (docs only until ops-box broadcast): `SAFE_GENESIS=true` constructors, then Davis HW-signs `SafeGenesisBatch.s.sol` calldata with `EXPECTED_SAFE` = hardware EOA `0x4583F9b7a06aB8B5b7B4A7dD27e774356015d406` (no Gnosis Safe this round). `pauseLaunches(false)` LAST after VerifyGenesis. New Guardian/Factory addresses stay empty in `deployments/arc-testnet-isolated.json` — do not invent them. Historical Instant/Fair hashes remain in `deployments/arc-testnet.json` / `arc-testnet-journey.json` (`superseded: true`). `claimedProdPath` stays false until Instant/Fair smoke on the new deploy + Turnstile + wallet UI ACs. Keep #16 open.

**This amendment (PR #52 isolated EOA-guardian runbook):** Draft #52 documents the redeploy role map (LaunchSigner `0xd880BD31948Ffc89E8D26C6ac90f98F825E56E9e`, PricingSigner `0x346363d14E6Acf1b05CA8Aa22F7E06a201A69a76`, Keeper `0xf2105235d0a74969f229deb72d3C8C578643147F`) and public web env only (`NEXT_PUBLIC_WALLETCONNECT_ID`, `NEXT_PUBLIC_TURNSTILE_SITE_KEY`). Constructors happen off-PR on the ops machine. No invented tx hashes. Factory V1 economics unchanged. No 5042.

**Protocol release:** `0.3.4` (`v0.3.4`, `docs/version.json`). **Factory version:** V1 (`FACTORY_VERSION = 1`) — immutable, not the protocol semver.

**This amendment (PR #52 rebase onto `922f909` / #48 handbook after #80 / #81 / #46 / #75 / #44 / #70):** Draft #52 sits on squash-merged **#48** handbook (`922f909`), Industrial Forge (#80), docs-only CI path filter (#81), frontend observability (#46), restricted-access UX (#75), production E2E release gate (#44), sanctions ops freshness (#70), #17 evidence docs (#79), operator policy (#68), trusted geo/IP (#67), and exact official-list OFAC screening (#66), plus #49 UI QA, #42 full GitHub CI extras, and #50 page-budget. Handbook body search, badge matrix, nav blurbs, and `docs:links` stay. `.github/workflows/ci.yml` is identical to `origin/main` (no decide-tier rewrite; keep #81 `ci-decide` / `ci-ok`, #48 docs-copy/visual, #46 `obs-ui`, #75 restricted jobs, and #44 `e2e-release-gate`). Indexer `refreshQuotes` still avoids `QuoteAssetRegistry.get()` as a 13-field tuple (viem `Number()` coerced the USDC address). It uses batched `count`/`list` plus `isUsdPegOne`/`isEnabled` and ERC-20 `decimals`/`symbol`/`name`. Lean public-RPC knobs (`INDEXER_START_BLOCK`, `INDEXER_MAX_BLOCK_SPAN`, `INDEXER_LEAN_LOGS`) sit beside `#68` `gateProtectedWrite` and `#70` `applySanctionsOpsGate` on admit/authorize/quote/upload. Claimed Factory + Instant/Fair explorer evidence is unchanged. `pnpm arc:prod-web` records honest PROD-path blockers in `deployments/arc-testnet-prod-path.json` (`claimedProdPath: false` — Turnstile, isolated signer ≠ deployer ≠ Keeper, injected-only wallet, no Safe). Do not invent those keys. Architecture and tokenomics unchanged.
...
**This amendment (handbook, Refs #14, squash-merged #48 / `922f909`):** landed on `origin/main`. Protocol identity stays **0.3.4** / Factory **V1**. Handbook preserves Forge chrome (`rx-kicker` / `rx-link`) additively with body search, badge matrix, and nav blurbs. Handbook muted copy uses the approved AA floor (`text-zinc-400`); leftover `text-zinc-500` on `/docs` failed `web-qa` `assertNoSubAaMutedText` and is raised, not the assertion. The four #14 AC surfaces landed with the squash. **#54 is not a prerequisite** and is not stacked. Issue **#14** close is a founder decision. Architecture and tokenomics unchanged.
