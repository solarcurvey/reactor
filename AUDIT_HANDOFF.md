...
**This software has not been audited.** Treat every contract as hostile-unreviewed. Do not deploy to Arc Mainnet (5042). No production / Safe-genesis claim. #16 rehearsal recorded Factory `0xB48D1B397834eBcccb8961041d827487097e0535` on Arc Public Testnet (`5042002`) with Blockscout-confirmed Instant + Fair smoke and LOCAL authorize Instant RHRSL + Fair RHRFL (`deployments/arc-testnet.json`, `arc-testnet-journey.json`, `docs/deployments.md`). Guardian is the disposable deployer EOA, not a Safe. Instant/Fair quote is Mock USDC-6, not canonical `0x3600…0000`. LOCAL authorize is not full PROD (no Turnstile / isolated signer). Keep #16 open for human AC.
