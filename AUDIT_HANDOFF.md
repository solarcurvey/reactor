...
| Indexer / Top-10 | `block.timestamp` only for onchain/windowed marks; durable poolId→token; event writes + cursor one transaction; material vs irrelevant inactivity. Offchain ms columns (`admission_hits.ts`, `issuance_bucket.updated_ms`, `leader_locks`, `keeper_operations.ts`, `alerts.ts`) are `BIGINT` (schema v6) — Postgres INTEGER overflows `Date.now()` | `indexer.persist.test.ts`, `tick-atomic.test.ts`, `pg-ms-timestamps.test.ts`, `Top10Api.t.sol` |
