...
**This amendment (repo publicization / public-fork Actions, Refs #72):** inventory + documentation + least-privilege workflow permissions only. Visibility stays private. History is not rewritten. Personal-mailbox `Co-authored-by` trailers remain until a founder-approved `filter-repo` remap. Full-history gitleaks/trufflehog on 2026-09-12 found no live credentials. Compatible with #69 CI cost / concurrency. Architecture and tokenomics unchanged. Re-audit `/docs/publicization` immediately before any visibility change; do not treat this as a protocol change.
...
| Repo publicization / public-fork Actions (P1 ops) | Visibility stays private until founder instruction. No history rewrite in this pass. Workflow `contents: read` + `persist-credentials: false`; no `pull_request_target`. Compatible with #69. Re-scan secrets + emails before any visibility change. | `docs/publicization.md`, `scripts/ci-public-harden.test.ts`, `.github/workflows/*.yml` |
