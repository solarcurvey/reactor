...
| Read-path budgets (#37) | Board/search/token/Launch/Rewards/REACTOR/CORE/quote/wallet stay O(1) or O(page) on a 4k-market seed. SSE patches, does not invalidate the board. No refetch-on-focus for expensive reads. Live `POST /quote` stays 30s fail-closed. Visible fast CI job `page-budget` in #73’s `.github/workflows/ci.yml`. Issue stays open until merge + post-merge verify. | `page-budget.test.ts`, `indexed.test.ts`, `rpc-batch.test.ts`, `.github/workflows/ci.yml` |
