...
| Production UI E2E gate | `next build`/`next start` + EIP-1193 fixture **and** MetaMask/Rabby-style MV3 extension (no mainnet keys). Desktop + iPhone-class + narrow-Android. BUY/SELL/nested BUY+SELL (`UserRouteExecutor` target/calldata/result), bonding/graduated/launch/rewards/Dev Buy, full ticket lifecycle, lock/switch/disconnect, revert, allowance, quote TTL, dropped/replaced tx, double-submit, wrong-chain, reject. Ticket/launch phase testids are `sr-only` (E2E reads `data-phase`; #49 goldens unchanged). Rewards defers wallet chrome until mount. Shared `console.error` / `pageerror` fixture fails teardown (documented allowlist: Chromium HTTP 503 on `/api/launch-pricing` during Dev Buy authorize-down; Next.js RSC prefetch fallback on iPhone WebKit / Firefox; WebKit access-control on mock indexer `:18448`). `docs:check` rejects leftover conflict markers. Refs #35 / Refs #15 only. | `apps/web/e2e/release/{journeys,edge,extension}.spec.ts`, `e2e/harness/console-gate.ts`, `playwright.release.config.ts`, `.github/workflows/ci.yml` job `e2e-release-gate` |
