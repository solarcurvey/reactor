...
**This amendment (Industrial Forge brand, Refs #55):** founder-locked Direction C is the production visual identity — tokens, vessel mark, wordmark, favicon/OG/app icons, product chrome, and voice. After #75, `/restricted` and deny banners use the same system: heat `rx-kicker`, 0–4px radius, no cyan; amber stays the semantic deny warning. #55 stays open for independent audit. Issue #36 is closed (`ad7b457`, post-merge `34729758795`); `web-qa` remains required and must cover this approved-C state **plus #44 launch/trade phase and wallet chrome** (do not weaken that gate; recapture overlapping goldens after a wallet rebase). No contract, fee, Factory, CORE, or routing change. See `/docs/brand`.
