...
**This amendment (full GitHub CI extras, Refs #17, rebased onto #73):** leftover #17 gates land on the #69 three-tier `ci.yml` — not a second workflow. `docs:links` is in `test:lib` and a full-only `docs-links` job. Playwright smoke + interactive is full-only job `web`. Foundry Attack / CREATE2 / size-guard, Postgres, production Next security, and live-toasts stay on the #73 jobs. A skipped job is not a pass. Architecture and tokenomics unchanged. See `/docs/ci`.
