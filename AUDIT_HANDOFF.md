...
**This amendment (Discover/search backend, Refs #40, PR #45 draft):** Homepage and `/search` are `GET /markets?q=&board=&cursor_*` (`useMarketsInfinite`). Chip predicates are indexer SQL. Schema **v12** adds quote-side `liquidity_usd6` and `change_24h_bps` (empty → UI `—`, not 0%). Rebased onto `origin/main` `922f909` after squash-merged **#48** handbook and **#80** Industrial Forge (on **#81** / **#46** / **#75** / **#44** / **#70** / **#79** / **#68**). Preserved handbook body search / badge matrix / nav blurbs plus Forge tokens/components + #81 `ci-decide` / `ci-ok` + #46 `reactorFetch` / error boundaries / release SHA + #75 `/restricted` write-lock / denied-policy a11y/reflow / four-state policy matrix (launch/trade not taken wholesale). `useMarketsInfinite` / `useFeaturedMarkets` honor `?inject=indexer|empty`. `filterMarkets` is not in the web app. Visual hash gate is `docs/visual-accept.json`; Chromium pixels stay with merged #49/#80 until recapture. Architecture and tokenomics unchanged. **#39 is reopened** after merged #46 — remaining gate is live configured telemetry-provider proof (in-process `vendor-proof` does not close it). **#40 cannot close while #39 remains open.** Do not close #40, #15, #39, or #55 from this file.
