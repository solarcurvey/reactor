...
<<<<<<< HEAD
<<<<<<< HEAD
**This amendment (Industrial Forge brand, Refs #55):** founder-locked Direction C is the production visual identity — tokens, vessel mark, wordmark, favicon/OG/app icons, product chrome, and voice. Rebased onto main `ff444cb` after **#81**. After #81, docs-only/trivial ready PRs stay on the cheap `ci-decide` path; this brand PR is code and still launches `web-qa` / `obs-ui` / the rest of the full gate. After #75, `/restricted` and deny banners use the same system: heat `rx-kicker`, 0–4px radius, no cyan; amber stays the semantic deny warning. After #46, error boundaries keep release SHA + `traceId` and use heat kickers (no leftover cyan/pills). #55 stays open for independent audit. Issue #36 is closed (`ad7b457`, post-merge `34729758795`); `web-qa` remains required and must cover this approved-C state **plus #44 launch/trade phase and wallet chrome** (do not weaken that gate; recapture overlapping goldens after a wallet rebase). No contract, fee, Factory, CORE, or routing change. See `/docs/brand`.
=======
**This amendment (handbook rebase, Refs #14, draft PR #48):** restacked onto `origin/main` `cc82cd4` (merged #70). Protocol identity stays **0.3.3** / Factory **V1** from current `main`. Handbook nav includes landed `/docs/sanctions-ops`, `/docs/sanctions-runbook`, `/docs/incident-response`. Issue **#14 stays open**. Do not claim AC complete. Architecture and tokenomics unchanged.
>>>>>>> 74df148 (Record handbook rebase onto main #70.)
=======
**This amendment (handbook rebase, Refs #14, draft PR #48):** restacked onto `origin/main` `4207356` (merged #75 after #44 `b190e86`). Protocol identity stays **0.3.3** / Factory **V1** from current `main`. Handbook nav includes landed `/docs/restricted-access`, `/docs/sanctions-ops`, `/docs/sanctions-runbook`, `/docs/incident-response`. Issue **#14 stays open**. Do not claim AC complete. Architecture and tokenomics unchanged.
>>>>>>> c24cad0 (Fold #75 restricted-access into the handbook after rebase.)
