...
**This amendment (handbook rebase, Refs #14, draft PR #48):** restacked onto `origin/main` `4915f3e` (squash-merged **#80** Industrial Forge). Protocol identity stays **0.3.4** / Factory **V1** from current `main`. Handbook preserves Forge chrome (`rx-kicker` / `rx-link`) additively with body search, badge matrix, and nav blurbs. Handbook muted copy uses the approved AA floor (`text-zinc-400`); leftover `text-zinc-500` on `/docs` failed `web-qa` `assertNoSubAaMutedText` and is raised, not the assertion. The four #14 AC surfaces are **implemented; full hosted proof pending**. **#54 is not a prerequisite** and is not stacked. Issue **#14 stays open**. Do not merge #48. Architecture and tokenomics unchanged.
