...
| Indexer / Top-10 | Indexed markets + ValuationService snapshot; no per-request Factory RPC. Event journal schema v8. `tokens.current_supply` is schema v9 (#23). Top-10 candidate tables are schema v10. Ranks from persisted `current_supply` (not TokenCreated − SelfBurn). Offchain ms columns are `BIGINT` (schema v6) | `top10-rank.test.ts`, `ingest.valuation.test.ts`, `tick-atomic.test.ts`, `pg-ms-timestamps.test.ts`, `Top10Api.t.sol` |
