...
contract SafeGenesisBatch is Script {
    function run() external view {
        ...
        _log("setPricingSigner", address(auth), abi.encodeWithSelector(ReactorGuardian.setPricingSigner.selector, pricingSigner));
        _log("setUsdc", registry, abi.encodeWithSelector(QuoteAssetRegistry.setUsdc.selector, usdc));
        _log(
            "register USDC",
            registry,
            abi.encodeWithSelector(
                QuoteAssetRegistry.register.selector, usdc, "USDC", "USD Coin", uint8(6), "", QuoteAssetRegistry.Category.Stablecoins
            )
        );
        _log("setUsdPegOne USDC", registry, abi.encodeWithSelector(QuoteAssetRegistry.setUsdPegOne.selector, usdc, true));
        _log("setBuybackRoute USDC", registry, abi.encodeWithSelector(QuoteAssetRegistry.setBuybackRoute.selector, usdc, true, false));
        ...
        _log("setAdapter user", address(auth), abi.encodeWithSelector(ReactorGuardian.setAdapter.selector, userAdapter, true));
        _log("setAdapter protocol", address(auth), abi.encodeWithSelector(ReactorGuardian.setAdapter.selector, protocolAdapter, true));
        ...
        _log("setProtocolVault selfBurn", router, abi.encodeWithSelector(ReactorRouter.setProtocolVault.selector, selfBurn, true));
        _log("setProtocolVault flywheel", router, abi.encodeWithSelector(ReactorRouter.setProtocolVault.selector, flywheel, true));
        _log("setProtocolVault coreBuyback", router, abi.encodeWithSelector(ReactorRouter.setProtocolVault.selector, coreBuyback, true));
        _log("setProtocolVault protocolAdapter", router, abi.encodeWithSelector(ReactorRouter.setProtocolVault.selector, protocolAdapter, true));
        ...
        _log("pauseLaunches(false) LAST after VerifyGenesis", address(auth), abi.encodeWithSelector(ReactorGuardian.pauseLaunches.selector, false));
        console2.log("Deployer cannot call any of these. Ops count = 28.");
    }
