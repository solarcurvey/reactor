...
Visual identity is **Industrial Forge** (founder-locked Direction C). Vessel mark, slag / steel / heat. See [`docs/brand.md`](docs/brand.md). Issue #55 stays open until independent audit.

> This repository is **not audited**. Do not deploy to Arc Mainnet (chain 5042).

Protocol release **0.3.5** (`v0.3.5`). Factory **V1** (`FACTORY_VERSION = 1`, immutable). Source of truth: `docs/version.json`. Changelog: `CHANGELOG.md`. Docs policy: `CONTRIBUTING.md`.
...
GitHub Actions is three-tier (fast PR / full merge-candidate / main). Docs-only PRs stay on the cheap gate even when ready for review; `ci-full` / dispatch **full** / `main` still force the heavy matrix. Full gate includes Foundry + Attack + CREATE2 + size guard, `docs:links`, Playwright smoke, visual/a11y `web-qa`, production Next + wallet E2E (`e2e-release-gate`), production Next security, live-toasts, and Postgres. See [`/docs/ci`](docs/ci.md).
