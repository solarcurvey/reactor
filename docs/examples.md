...
Expect `ok`, `minOut` > 1, `feeLegs[]` from the scored winner, hop `kind`s (`OFFICIAL_REACTOR_V4` / `EXTERNAL_V4_HOOKLESS` / `BONDING_CURVE`). Nested official USDC → ZEC → ZCAT → CAT lists two 3.5% legs (`aggregateProtocolImpactBps` **688**). Hops / `amountOut` / `minOut`s are one candidate. SELL also returns `minQuoteOut` in **quote** units (not `amountIn`). Submit `tx.to` + `tx.data`. A failed preview returns `ok: false`, not a dust floor.
