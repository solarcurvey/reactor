...
Production testnet rehearsal (`REACTOR_ENV=PROD` against chain 5042002) uses the same authorize path. Factory code exists on 5042002 (`docs/deployments.md`). `pnpm arc:wallet-harness` is the indexer authorize + `POST /quote` + `UserRouteExecutor` path (LOCAL authorize is labeled, not full PROD Turnstile). `pnpm arc:smoke` is Guardian-signed Factory Instant + Fair. Keep #16 open for human AC.
