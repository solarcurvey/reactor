...
- Operator policy (recovered wallet proof + trusted geo) is enforced on REACTOR-operated write/authorization APIs only. `GET /operator-policy/status` is the public decision read for hosted UX. `/restricted` is the dedicated launchpad state. Public market/docs reads and onchain contracts are not this gate. [Operator policy](/docs/operator-policy) · [Restricted access](/docs/restricted-access).
