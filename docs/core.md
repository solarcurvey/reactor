...
Live UI: a bottom-right toast appears when the indexer has committed `BuybackExecuted` / `COREBurned` (SSE `core`, `confirmed`, `id > hello.head`). Same tx is one notice. This is not an onchain oracle and not a Top-10 event — CORE never ranks.
