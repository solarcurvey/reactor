...
Production integration (`pnpm test:operator-policy-http`, CI job `operator-policy-http`) starts the real indexer and production Next and sends the blocked-wallet / blocked-geo / allow / stale / spoofed-client matrix through `/quote`, `/launch/authorize`, `/upload`, and Next `/api/launch-pricing`. Helper units remain on `test:lib`.
