...
`POST /upload` stays on its own **2MB** image stream cap. The public Next BFF `POST /api/launch-pricing` applies the same default 16KiB / hard-max 64KiB JSON cap before proxying. `POST /api/telemetry` uses the same JSON cap, re-redacts, and rate-limits (**40 POSTs / 10s / IP** in genuine production; review-fixture / `REACTOR_TELEMETRY_RELAXED=1` artifacts raise the cap). A 429 is expected backpressure, not a user-visible outage. `GET /api/version` returns protocol, build SHA, `reactorEnv`, `chainId`, `chainName`, and `buildTimestamp` (no secrets).
