...
| `solidity + size-guard` | full / main | `FOUNDRY_PROFILE=ci forge test` + Attack suite + CREATE2 `test_hookBits` + `pnpm size:guard` + #51 dual-relayer rehearsal scripts + pin-check of committed CRE evidence / tenant-blocker (`cre-workflow-simulate.ts --verify`). That step is **not** a successful `cre workflow simulate`. |
...
| #39 (merged #46; **open/reopened**) | Observability | `obs-ui` (full). Units also run in `test:lib` on the fast gate. Folded from `observability.yml` — do not restore a second workflow. **#39 stays open** — merged #46 landed repo CI; close still requires the real configured telemetry-provider symbolication proof. |
...
| #51 (PR #54) | AutomationGateway Foundry + dual-relayer rehearsal + committed CRE evidence | Fast + Solidity: `foundry-targeted` `forge test` (includes `AutomationGateway` / `MaintenanceFailover`). Full/main: same `forge test` plus rehearsal scripts and `cre-workflow-simulate.ts --verify` on `solidity + size-guard`. `--verify` pin-checks committed courier hashes + `cre-tenant-blocker.json`. It does **not** run a secret-bearing tenant simulate. Authenticated CRE simulate is **optional interoperability** after **#83**; not a #54 merge blocker. Cheap verify also in `test:lib`. No second workflow. **#51 stays open** through #83 testnet/failover + post-deploy keeper. |
