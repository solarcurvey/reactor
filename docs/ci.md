...
| #62 (PR #68) | Operator policy gate | Cheap units in `test:lib` (`sanctions-policy`, `wallet-proof`, `operator-policy`, BFF, status GET). Official `#61` `sanctions.ts` binds via `tryBindOfficialPolicyPlugins`. Official #65 read: `GET /operator-policy/status`. No new workflow. |
