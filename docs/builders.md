...
4. **Media** — `POST /upload` (operator policy via `x-reactor-wallet`, then stream 2MB, sharp, SigV4 remote). Store the returned `publicUrl`. Object key is `m/<id>.webp` (matches `/m/<id>.webp`). No base64 onchain. Do not render creator image/name/description as HTML. URL / media allowlists: [Browser security](/docs/web-security).
