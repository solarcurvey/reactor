...
2. **Quote** — `POST /quote`. Use the ticket. One `UserRouteQuoter` call per candidate. Do not invent hops. Do not mix another candidate's preview onto the selected path. Do not set `minOut` to 0 or 1. On SELL, submit the ticket’s `minQuoteOut` (first-leg quote units) and `minOut` (final USDC). JSON body is capped at **16KiB** default / **64KiB** hard max (413 if over, including chunked).
