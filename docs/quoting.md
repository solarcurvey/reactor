...
> User quotes are a **single `eth_call`** over the whole route. The ticket is one candidate. `minOut` is never 0 or 1.

`UserRouteQuoter.previewBuy` / `previewSell` **executes hops in-place** (no nested revert that would undo intermediate balances) and then reverts `PreviewRoute`. The indexer also sends ERC-20 **state overrides** that credit the quoter — not the user wallet — on candidate `balanceOf` slots. Nested hops do **not** need intermediate wallet balances. The user can hold only USDC.

`POST /quote` is a REACTOR-operated write-assist path: the [operator policy](/docs/operator-policy) gate (recovered EIP-191 wallet proof + trusted geo) runs **before** any ticket / `tx` payload. `recipient` is overwritten to the recovered signer. When the decision is deny or unavailable, the launchpad does not prompt a wallet and does not treat a client “clear” flag as authority. Public `GET` market/docs reads are not gated. See [Restricted access](/docs/restricted-access).
...
`minQuoteOut` is quote units. It is never launch-token `amountIn`. A missing first-leg (`hopOuts` empty) or malformed `plannedHops+1` preview fails the ticket (`ok: false`, no `tx`). Routed SELLs do not rebuild `hopOuts` from sequential hop sims — they use the selected `PreviewedRoute` only.
...
- Submit `tx.to` + `tx.data` from the ticket
- Apply your slippage locally; refuse `minOut` ≤ 1
- Preserve hop `kind`, `feeLegs[]`, and `aggregateProtocolImpactBps`
- JSON body cap: **16KiB** default / **64KiB** hard max → **413**
- Quote TTL is **30 seconds** (`QUOTE_TTL_SEC`) — do not hold a ticket across a minute

See [Routes](/docs/routes), [Fees](/docs/fees), [Examples](/docs/examples), [Troubleshooting](/docs/troubleshooting).
