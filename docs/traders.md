...
5. Nested official legs appear in `feeLegs[]`. A routed SELL ticket’s `minQuoteOut` is slipped first-leg **quote** out, not tokenIn. A failed preview is **unavailable** — never `minOut` 0 or 1.
