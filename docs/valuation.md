...
Accepted and rejected observations plus the `kind=consensus` row are persisted for `/pricing/health` and the watchdog. Schema **v10** adds `external_price_marks.kind` (#30). Schema **v9** is `tokens.current_supply` (#23); #30 reserves that slot. v8 production DBs `ALTER` + backfill.
