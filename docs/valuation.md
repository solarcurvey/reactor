...
Accepted and rejected observations plus the `kind=consensus` row are persisted for `/pricing/health` and the watchdog. Schema **v10** adds `external_price_marks.kind` (real post-#27 v8 DBs `ALTER` + backfill; v9 reserved for #23 `current_supply`). `route_venues.last_price_quote_x18` is column-gated (no `schema_migrations` id) so the train stays **#23 (v9) → #30 (v10) → #29 (v11)**.
