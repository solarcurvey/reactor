...
Accepted and rejected observations plus the `kind=consensus` row are persisted for `/pricing/health` and the watchdog. Schema **v9** adds `external_price_marks.kind` (v8 production DBs `ALTER` + backfill).
