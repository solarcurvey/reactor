...
5. **Designated Keeper runs maintenance.** One lease (`leader_locks.lease_until` in **milliseconds**, `BIGINT`) with renew + acquire-generation fence (TTL is failover, not a tick budget). Simulated minOut. Cannot configure. CRE does not rank Top-10. Signed-job `AutomationGateway` / `MaintenanceJob` bind is draft **#54**, not current `main`. See [Keeper](/docs/keeper) · [Automation](/docs/automation).
