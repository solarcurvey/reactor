...
13. The consumer UI **fails visible** on indexer / RPC / `POST /quote` outages (including 429/413/5xx, stale/expired tickets, no route). A down indexer is not an empty board. A failed quote is not a 0/1 `minOut` ticket. Pricing and upload fail closed. SSE reconnect must not duplicate toasts. Review/QA-build `?inject=` is ignored in production. See [UI QA](/docs/qa).
14. This repo is **not audited**. Do not deploy to Arc Mainnet (5042).
15. **Repository visibility is an operator decision.** The tree may stay private. Public-fork Actions must not receive repository secrets or a writable `GITHUB_TOKEN`. Do not publicize without founder instruction. Residual pre-rewrite dangling SHAs are accepted; Support purge/GC is not a #72 AC. See [Repo publicization](/docs/publicization).
